...
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
│  PHASE 4: INTERACTIVE TESTS (KRITISCH!)                        │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ 1. Test-Setup: Auth Fixtures verfügbar?                     ││
│  │    → Prüfe: tests/fixtures/auth.fixture.ts existiert        ││
│  │ 2. Generate INTERACTIVE test requirements:                  ││
│  │    → Analysiere Feature-Typ (Toggle, Form, List, etc.)      ││
│  │    → Generiere test scenarios basierend auf Feature         ││
│  │ 3. Test Agent spawnen mit INTERAKTIVEN Anforderungen        ││
│  │    → Tests MÜSSEN klicken, nicht nur page-load prüfen!      ││
│  │ 4. Tests ausführen + Console-Errors nach Interaktionen     ││
│  │ 5. Bei Failures → Fix-Loop                                  ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
│  PHASE 5: INTERACTIVE QA + FIX-LOOP                             │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ 1. Login als Test-User (e2e-test@candidatecluster.local)   ││
│  │ 2. Navigate to feature page                                 ││
│  │ 3. BASELINE: Console errors check                           ││
│  │ 4. FOR EACH interactive element:                            ││
│  │    → Click/interact                                         ││
│  │    → Check console for NEW errors                           ││
│  │    → Screenshot if error found                              ││
│  │ 5. Fix-Loop wie bei UI Review                               ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
## Feature-to-Test Mapping (PFLICHT!)

**Analysiere das Feature und generiere passende Test-Szenarien:**

| Feature-Typ | Erkennungsmerkmale | Test-Anforderungen |
|-------------|-------------------|-------------------|
| **Toggle/Switch** | Switch, Checkbox, role="switch" | Click ON → verify state → Click OFF → verify → Console check each |
| **Form** | form, input, textarea | Empty submit → validation → valid submit → success → console |
| **List/Table** | map(), DataTable, Pagination | Empty state → loading → populated → pagination → console |
| **Modal/Dialog** | Dialog, Modal, role="dialog" | Open trigger → content visible → close → verify closed |
| **Button** | button, onClick, role="button" | Click → expected action → no console errors |
| **Navigation** | Link, href, router | Click → URL change → back button → console |
...
FEATURE-TYP: [Toggle/Form/List/etc.]
ROUTE: [URL der Feature-Seite]
INTERACTIVE ELEMENTS: [Liste der UI-Elemente]

PFLICHT-TESTS basierend auf Feature-Typ:
[Generierte Test-Anforderungen aus Mapping]

KRITISCH:
- Nutze auth fixture: import { test, loginAsAgency } from './fixtures/auth.fixture'
- JEDER Test muss Console-Errors nach Interaktionen prüfen
- NIEMALS nur page-load Tests erstellen
- Test-Datei: tests/[feature-name].spec.ts
```
...
PFLICHT-WORKFLOW:
1. Login: e2e-test@candidatecluster.local / E2E-Test-Password-2024!
2. Navigate to [URL]
3. BASELINE console check
4. FOR EACH element:
   - Interact (click/fill/submit)
   - Wait 500ms
   - Console check → Report NEW errors
   - Screenshot bei Error
5. Report fix_required: true wenn ANY errors found
```

## Test Credentials

```
Email: e2e-test@candidatecluster.local
Password: E2E-Test-Password-2024!
Agency: E2E Test Agentur
```
...
- ❌ **Tests erstellen die nur Page-Load prüfen**
- ❌ **QA ohne Interaktion mit UI-Elementen**
- ❌ **Console nur bei Page-Load checken, nicht nach Klicks**
...
- ✅ **Feature-Typ analysieren vor Test-Agent spawn**
- ✅ **Interactive Test Requirements generieren**
- ✅ **Console-Errors NACH jeder Interaktion prüfen**
- ✅ **Auth Fixtures für geschützte Routes nutzen**
